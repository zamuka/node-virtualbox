...
function snapshot(vmname, command, snapname, callback) {
  logging.info('"%s" snapshot "%s" on VM "%s"', command, snapname, vmname);
  vboxmanage('snapshot "' + vmname + '" ' + command + ' "' + snapname + '"', function(error, stdout) {
    if (error) {
      if (callback) {
        callback(error) 
      } else { 
